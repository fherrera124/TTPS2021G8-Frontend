...
import { StudyCanceledListService } from '../../../_services/study.service';
import { DetailStudyModalComponent } from '../detail-study-modal/detail-study-modal.component';

let $: any = jQuery;
@Component({
  selector: 'app-study-canceled-list',
  templateUrl: './study-canceled-list.component.html',
})
export class StudyCanceledListComponent
  implements
  OnInit,
  OnDestroy,
  ISortView,
  ...
  constructor(
    ...
    public studyCanceledListService: StudyCanceledListService,
  ...
  ngOnInit(): void {
    ...
    this.filterForm();
    this.searchForm();
    this.studyCanceledListService.fetch();
    this.grouping = this.studyCanceledListService.grouping;
    this.paginator = this.studyCanceledListService.paginator;
    this.sorting = this.studyCanceledListService.sorting;
    const sb = this.studyCanceledListService.isLoading$.subscribe(res => this.isLoading = res);
  ...
  filter() {
    ...
    this.studyCanceledListService.patchState({ filter });
  }
  ...
  search(searchTerm: string) {
    ...
    this.studyCanceledListService.patchState({ searchTerm });
  }
  ...
  sort(column: string) {
    ...
    this.studyCanceledListService.patchState({ sorting });
  }

  // pagination
  paginate(paginator: PaginatorState) {
    this.studyCanceledListService.patchState({ paginator });
  }
